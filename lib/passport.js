...
async.waterfall([
    ...
    function(token, done) {
        generic.sendMailtoLocal(req, token, local, 'verify', done);
    }
    ], function(err) {
        if (err) {
        ...
        req.models.Local.exists({ email :  mail }, function(err, flag) {
            ...
            if (flag) {
                ...
                req.models.Local.create([{
                    ...
                    if (err) {return done(null, false, req.flash('registerMessage', 'Error in creating local user.'));  }
                    ...
                    req.models.User.create([{
                        ...
                        if (err){return done(null, false, req.flash('registerMessage', 'Error in creating user')); }
                        var user = u_created[0];
                        user.setLocal(local, function (err){
                            if (err) {return done(null, false, req.flash('registerMessage', 'Error in setting user-local relation'));};
                            req.logIn(user, function (err) {
                                if (err) return done(err);
                                sendVerificationMail(req, local, function () {
